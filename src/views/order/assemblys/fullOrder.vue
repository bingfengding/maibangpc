        ...
        <el-table-column            
         ...
         >                     
            <template slot-scope="scope">                               
                ...
                <div class="goods-exhibition" @click="goDetail(scope.row.id)" v-for="(item,index) in scope.row.proData" :key="index" v-if="scope.row.proData.length==1" style="clear:both">
                    <div class="goods-pic">
            ...
            <template slot-scope="scope">
                ...
                <div class="goods-operation">
                    ...
                    <div class="right-btn">
                        ...
                        <el-button type="primary" size="mini" v-if="scope.row.status==3||scope.row.status==5||scope.row.status==4||scope.row.status==2" @click="printOrder(scope.row.id)">打印订单</el-button >
                        <!-- <el-button type="primary" size="mini" @click="confirmGood(scope.row.id)" v-if="scope.row.status==2">确定发货</el-button >
                        <el-button type="primary" size="mini" @click="confirmGoods(scope.row.id,scope.row.status)" v-if="scope.row.status==3||scope.row.status==5">修改发货</el-button > -->
                    </div>
                </div>
...
#fullOrder >>> .el-table th:nth-child(1){
    padding-left: 20px;
    min-width: 280px;
}

/* #fullOdrer >>> .el-checkbox__inner:nth-child(2){
    width: 10px;
} */
/*
#fullOrder >>> .el-checkbox__input:nth-child(2){
    position: absolute;
    top: 0;
    left: 10px;
} */

#fullOrder >>> .el-table th:nth-child(5){
    text-align: right;
    padding-right: 20px;
}
...
#fullOrder >>> .el-table::before{
    height: 0;    
    z-index: 0;
}
#fullOrder >>> .el-table--border::after, .el-table--group::after, .el-table::before{
